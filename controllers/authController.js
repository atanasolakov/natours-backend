...
exports.protect = catchAsync(async(req,res,next) => {
 ...
 console.log(token, 'TOKENA')
  ...
  //3 Check if user still exists
   ...
   if(!freshUser) {
    return  next(new AppError('The user belonged to the token no longer exist', 401))
   }
...
exports.restrictTo = (...roles) => {
  return (req, res, next) => {
     if(!roles.includes(req.user.role)) {
      return next(new AppError('Access denied for this operation', 403))
     }
     next()
  }
}
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  // Check iuf posted current password is correct
     if(!(await user.correctPassword(req.body.passwordCurrent, user.password))) {
       return next(new AppError('Your current password is wrong', 401))
     }
  // if pass is correct update it
  user.password = req.body.password;
